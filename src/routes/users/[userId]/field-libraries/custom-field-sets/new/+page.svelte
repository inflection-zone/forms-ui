<script lang="ts">
	...
	function saveCustomField() {
		...
		console.log('Creating custom field:', customField);
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center space-x-4">
					<button 
						...
						title="Back to Custom Field Sets"
					>
					...
					<div>
						...
						<p class="text-gray-600 dark:text-gray-400 mt-1">
							Build a reusable custom field with multiple components
						</p>
					</div>
				</div>
					...
					<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
						...
						{#if customField.subFields.length === 0}
							<div class="text-center py-8">
								...
								<p class="text-gray-500 dark:text-gray-400 mb-4">No sub-fields added yet</p>
								<button
									onclick={addSubField}
								...
								{#each customField.subFields as subField, index}
									<div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
										...
										<div class="grid grid-cols-2 gap-4 mb-3">
												<!-- Sub-field Name -->
												<div>
													<label for="subfield-name-{index}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
														Name *
													</label>
													<input
												...
												<!-- Sub-field Type -->
												<div>
													<label for="subfield-type-{index}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
														Type
													</label>
													<select
														id="subfield-type-{index}"
										...
										<div class="border-t border-gray-200 dark:border-gray-600 pt-3">
											...
											<div class="grid grid-cols-2 gap-4">
												...
												{#if subField.type === 'text' || subField.type === 'textarea'}
																	<div>
																		<label for="min-length-{index}" class="block text-xs text-gray-500 dark:text-gray-400">Min Length</label>
																		<input
																			id="min-length-{index}"
																	...
																	<div>
																		<label for="max-length-{index}" class="block text-xs text-gray-500 dark:text-gray-400">Max Length</label>
																		<input
																			id="max-length-{index}"
												...
												{#if subField.type === 'number'}
																	<div>
																		<label for="min-value-{index}" class="block text-xs text-gray-500 dark:text-gray-400">Min Value</label>
																		<input
																			id="min-value-{index}"
																	...
																	<div>
																		<label for="max-value-{index}" class="block text-xs text-gray-500 dark:text-gray-400">Max Value</label>
																		<input
																			id="max-value-{index}"
								...
								{#if customField.subFields.length > 0}
									<div class="grid gap-4" class:grid-cols-1={customField.columns === 1} class:grid-cols-2={customField.columns === 2} class:grid-cols-3={customField.columns === 3}>
										{#each customField.subFields as subField}
															<div class="space-y-1">
																<label for="preview-{subField.id}" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
																	{subField.name}
												...
												{:else if subField.type === 'checkbox'}
													<div class="flex items-center space-x-2">
														<input type="checkbox" class="h-4 w-4 text-orange-600 rounded" disabled />
